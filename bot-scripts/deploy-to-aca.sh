#!/bin/bash
#
# Deploy OpenClaw image to Azure Container Apps.
#
# Usage:
...
#
# Examples:
#   ./deploy-to-aca.sh openclaw my-resource-group v2026.1.24-abc1234
#   ./deploy-to-aca.sh openclaw my-resource-group latest
#
...
# Extract ACR name from current image (e.g., myacr.azurecr.io/openclaw:tag -> myacr)
ACR_LOGIN_SERVER=$(echo "$CURRENT_IMAGE" | cut -d'/' -f1)
ACR_NAME=$(echo "$ACR_LOGIN_SERVER" | cut -d'.' -f1)

NEW_IMAGE="${ACR_LOGIN_SERVER}/openclaw:${IMAGE_TAG}"
